...
**Mounted Credentials:**
- Model provider auth tokens (filtered from `.env`, read-only)
...
```typescript
const allowedVars = ['CLAUDE_CODE_OAUTH_TOKEN', 'ANTHROPIC_API_KEY', 'OPENAI_OAUTH_ACCESS_TOKEN', 'CODEX_OAUTH_TOKEN', 'OPENAI_API_KEY'];
```
